...
function calculateRelevance(prompt: Prompt, useCase: string, context: string, industry: string): number {
    let score = 0;
    ...
    if (useCaseLower.includes('email') && prompt.department === 'Sales') score += 5;
    if (useCaseLower.includes('social media') && prompt.department === 'Marketing') score += 5;
    if (useCaseLower.includes('content') && prompt.department === 'Writing') score += 5;
    if (useCaseLower.includes('seo') && prompt.department === 'SEO') score += 5;
    if (useCaseLower.includes('blog') && (prompt.department === 'Writing' || prompt.department === 'Marketing')) score += 4;

    return score;
}

function generateReason(prompt: Prompt, useCase: string): string {
    const reasons = [
...
export async function recommendPrompts(request: HttpRequest, context: InvocationContext): Promise<HttpResponseInit> {
