...
// @version      3.0.0
// @description  Draggable floating button with persistent sidecar (minimize/restore/pin) for M365 Copilot
...
(function() {
    ...
    console.log('[Spark] Initializing M365 Copilot integration v3.0.0...');

    // ==========================================
    // CONFIGURATION
    // ==========================================
    const CONFIG = {
        libraryUrl: 'https://gray-ocean-059c8510f.3.azurestaticapps.net',  // Your SPARK library URL (Azure deployment)
        inputSelector: '#m365-chat-editor-target-element',
        sidecarWidth: '380px',  // Reduced from 500px for better UX
        sidecarMinWidth: 280,
        sidecarMaxWidth: 600,
        sidecarMinHeight: 400,
        messageType: 'SPARK_SEND_TO_COPILOT',
        minimizeToButtonMode: false,  // If true, sidecar minimizes to button badge instead of edge tab
        snapEdgeThreshold: 16  // Pixels from edge to snap
    };
    ...
    let buttonPosition = null;  // Will load from localStorage
    ...
    // Sidecar drag/resize state
    let sidecarDragging = false;
    let sidecarResizing = false;
    let sidecarDragOffset = { x: 0, y: 0 };
    let sidecarPosition = null;  // Will load from localStorage
    let sidecarSize = null;  // Will load from localStorage
    ...
    function updateFloatingButtonUI(state) {
        ...
        let buttonColor = 'linear-gradient(135deg, #667eea 0%, #764ba2 100%)';  // Default
        let badgeVisible = false;

        switch (state) {
            case 'open':
                ariaLabel = 'SPARK Prompt Library - Sidecar open';
                buttonColor = 'linear-gradient(135deg, #10b981 0%, #059669 100%)';  // Green
                break;
            case 'minimized':
                ariaLabel = 'SPARK Prompt Library - Sidecar minimized (click to restore)';
                buttonColor = 'linear-gradient(135deg, #f59e0b 0%, #d97706 100%)';  // Amber
                badgeVisible = true;
                break;
            case 'closed':
            default:
                ariaLabel = 'Open SPARK AI Prompt Library';
                buttonColor = 'linear-gradient(135deg, #667eea 0%, #764ba2 100%)';  // Purple
                break;
        }
    ...
    function createFloatingButton() {
        ...
        // Create badge for minimized state indicator
        ...
        button.addEventListener('click', (e) => {
            ...
            // Check if Shift+Click (opens context menu)
            if (e.shiftKey) {
                e.preventDefault();
            ...
            // If sidecar is minimized, restore it instead of opening library
    ...
    function showContextMenu(button) {
        ...
        const menuItems = [
            ...
            },
            {
                ...
                action: () => {
                    const helpText = `
SPARK AI Prompt Library - Quick Guide

🔹 Floating Button:
   - Click: Open library (or restore sidecar if minimized)
   - Drag: Move button anywhere
   - Right-click: Show this menu
   - Shift+Click: Show this menu

🔹 Context Menu Actions:
   - 📚 Open Library: Open SPARK in new tab
   - 👁️ Show/Hide: Toggle sidecar visibility
   - 🔄 New Conversation: Refresh page for clean start
   - 📍 Reset Position: Move button to default location
   - 🗑️ Clear Preferences: Reset all saved settings
   - ❓ Help: Show this guide

🔹 Sidecar Panel:
   - Drag header: Move panel
   - Drag corner: Resize panel
   - 📌 Pin: Keep open while working in Copilot
...
🔹 Button States:
   - Purple: Idle/closed
   - Green: Sidecar open
   - Amber with dot: Sidecar minimized

Positions and sizes are auto-saved!
                    `.trim();
                    alert(helpText);
                }
            }
        ...
        const rect = button.getBoundingClientRect();
        menu.style.left = (rect.left - 120) + 'px';  // Offset to the left
        menu.style.top = (rect.top + 70) + 'px';  // Below the button
    ...
    function createSidecar() {
        ...
        Object.assign(sidecar.style, {
            ...
            left: 'auto',
            right: `-${width}`,  // Start off-screen
            width: width,
            height: height,
            background: 'white',
            boxShadow: '-4px 0 24px rgba(0, 0, 0, 0.15)',
            zIndex: '9999',
            transition: 'none',  // We'll enable transitions selectively
            display: 'flex',
            flexDirection: 'column',
            fontFamily: 'system-ui, -apple-system, "Segoe UI", sans-serif',
            borderRadius: '12px 0 0 12px',
            overflow: 'hidden',
            resize: 'none'  // We'll add custom resize handles
        });
        ...
        pinBtn.addEventListener('mouseenter', () => {
            if (!sidecarPinned) {
                pinBtn.style.background = 'rgba(255, 255, 255, 0.25)';
            }
        });
        ...
        // Close button (now also minimizes)
        ...
        minimizedTab.addEventListener('mouseenter', () => {
            minimizedTab.style.right = '4px';
            minimizedTab.style.boxShadow = '-6px 0 20px rgba(102, 126, 234, 0.5)';
        });
        minimizedTab.addEventListener('mouseleave', () => {
            minimizedTab.style.right = '0';
            minimizedTab.style.boxShadow = '-4px 0 16px rgba(102, 126, 234, 0.3)';
        });
        ...
        console.log('[Spark] ✓ Draggable & resizable sidecar created (380px default, drag header, resize handle)');
    }
    ...
    function openSidecar(promptDetails) {
        ...
        // MATCH EXACT ORDER FROM ViewPage.jsx
        let html = `
        ...
        // 5. Example Output (IMAGES ONLY) - FIFTH
                ...
                <div style="margin-bottom: 20px; padding: 20px; background: rgba(236, 72, 153, 0.05); border-radius: 12px; border-left: 4px solid #ec4899;">
                    ...
                    <div style="display: grid; grid-template-columns: repeat(auto-fill, minmax(250px, 1fr)); gap: 12px;">
                        ${promptDetails.images.map((img, index) => {
                            const imgUrl = `${CONFIG.libraryUrl}/thumbnails/${img}`;
                            return `
                                <div style="position: relative; background: linear-gradient(135deg, #fdf2f8 0%, #fce7f3 100%); border-radius: 12px; overflow: hidden; border: 2px solid #ec4899; padding: 20px; text-align: center; cursor: pointer; transition: all 0.3s ease;" onclick="window.open('${imgUrl}', '_blank')" onmouseover="this.style.transform='translateY(-4px)'; this.style.boxShadow='0 8px 16px rgba(236, 72, 153, 0.3)';" onmouseout="this.style.transform='translateY(0)'; this.style.boxShadow='none';">
                                    <div style="font-size: 48px; margin-bottom: 12px;">🖼️</div>
                                    <div style="font-weight: 600; color: #831843; font-size: 14px; margin-bottom: 8px;">Example Output ${index + 1}</div>
                                    <div style="display: inline-block; background: white; color: #ec4899; padding: 8px 16px; border-radius: 20px; font-size: 12px; font-weight: 600; border: 1px solid #ec4899;">
                                        👁️ Click to View Image
                                    </div>
                                    <div style="margin-top: 8px; font-size: 10px; color: #9f1239; opacity: 0.7;">
                                        Opens in SPARK Library
                                    </div>
                                </div>
                            `;
                        }).join('')}
                    ...
                    <div style="margin-top: 12px; padding: 12px; background: rgba(236, 72, 153, 0.1); border-radius: 8px; font-size: 13px; color: #831843; line-height: 1.6;">
                        <strong>📝 Note:</strong> ${promptDetails.images.length} example output image${promptDetails.images.length > 1 ? 's are' : ' is'} available. Click any card above to view the full-size image in the SPARK library.
                    </div>
                </div>
            `;
        }
        ...
        requestAnimationFrame(() => {
            ...
            // Position sidecar based on saved preferences or default
        ...
        showNewPromptGlow();  // Glow when new prompt is sent
    ...
    function minimizeSidecar() {
        ...
        // Hide sidecar off-screen to the right
        const currentWidth = sidecar.offsetWidth;
        sidecar.style.left = window.innerWidth + 'px';
        sidecar.style.right = 'auto';

        // Always hide overlay when minimizing (even if it was already hidden from pin)
    ...
    function restoreSidecar() {
        ...
        requestAnimationFrame(() => {
            ...
            // Restore to saved position or default to right
            if (sidecarPosition && sidecarPosition.left) {
                sidecar.style.left = sidecarPosition.left;
                sidecar.style.top = sidecarPosition.top;
                sidecar.style.right = 'auto';
    ...
    function insertPromptText(promptText) {
        ...
        if (!inputElement) {
            ...
            console.log('[Spark] All contenteditable elements:', document.querySelectorAll('[contenteditable="true"]'));

            // Last resort - just find ANY contenteditable
        ...
        try {
            ...
            // NUCLEAR OPTION: Multiple insertion methods simultaneously
            ...
            // Method 3: Set on parent
            if (p !== inputElement) {
                inputElement.textContent = formattedPrompt;
            }

            // Method 4: Dispatch ALL possible events
            const allEvents = [
            ...
            // Method 5: Try to find and call React's setValue
            try {
                ...
                if (reactKey) {
                    console.log('[Spark] Found React key:', reactKey);
                    const reactInstance = inputElement[reactKey];
                    console.log('[Spark] React instance:', reactInstance);

                    // Try to find onChange handler
                    if (reactInstance && reactInstance.onChange) {
                        console.log('[Spark] Calling React onChange...');
            ...
            console.log('[Spark] ✓ Insertion complete - checking in 1 second...');
            ...
            setTimeout(() => {
                ...
                console.log('[Spark] Preview:', currentText.substring(0, 100));

                if (currentText.length > 100) {
                    console.log('[Spark] ✅ ✅ ✅ SUCCESS! TEXT IS IN THE CHATBOX!');
                } else {
                    console.error('[Spark] ❌ FAILED - Text not persisting');
                    console.log('[Spark] DOM innerHTML:', p.innerHTML.substring(0, 200));
                    console.log('[Spark] DOM textContent:', p.textContent.substring(0, 200));

                    // Show what we found
                    alert('⚠️ Insertion may have failed. Check console for details.\n\nFound input: ' + (inputElement.id || inputElement.className));
                }
            }, 1000);
    ...
    function setupMessageListener() {
        ...
        console.log('[Spark] ✓ Message listener ready');
    }
    ...
    function init() {
        ...
        setTimeout(() => {
            ...
            console.log('[Spark] ⚡ Integration v3.0.0 complete!');
            console.log('[Spark] ✓ Draggable floating button');
            console.log('[Spark] ✓ Draggable & resizable sidecar (380px default)');
